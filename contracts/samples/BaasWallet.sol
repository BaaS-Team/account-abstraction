 ...
 */
contract BaasWallet is IBLSAccount, IAccountRecovery, BaseAccount, SignatureDecoder, UUPSUpgradeable, Initializable {
    ...
    event BaasWalletInitialized(IEntryPoint indexed entryPoint, address indexed owner);
    ...
    function _initialize(address anOwner, address server, uint256[4] memory aPublicKey) internal virtual {
        ...
        emit BaasWalletInitialized(_entryPoint, owner);
    }
    ...
    function executeBatch(address[] calldata dest, bytes[] calldata func) external {
        _requireFromEntryPointOrOwner();
        require(dest.length == func.length, "BaasWallet: Wrong array lengths");
    ...
    function _requireFromEntryPointOrOwner() internal view {
        require(msg.sender == address(entryPoint()) || msg.sender == owner, "BaasWallet: Not Owner or EntryPoint");
    }

    /// implement template method of BaseAccount
    function _validateAndUpdateNonce(UserOperation calldata userOp) internal override {
        require(_nonce++ == userOp.nonce, "BaasWallet: Invalid nonce");
    }
    ...
    {
        (userOp, userOpHash);
        require(userOpAggregator == address(aggregator), "BaasWallet: Wrong aggregator");
        return 0;
    }
