...
export async function createBaasWallet (
  ethersSigner: Signer,
  aggregator: string,
  accountOwner: string,
  server: string,
  ...
  dkimService: string,
  publicKey: any[4],
  _factory?: BaasWalletFactory
):
  Promise<{
    proxy: BaasWallet
    accountFactory: BaasWalletFactory
    implementation: string
  }> {
  const accountFactory = _factory ?? await new BaasWalletFactory__factory(ethersSigner).deploy(entryPoint, aggregator, dkimService)
  const implementation = await accountFactory.accountImplementation()
  ...
  const proxy = BaasWallet__factory.connect(accountAddress, ethersSigner)
  return {
    implementation,
    accountFactory,
    proxy
