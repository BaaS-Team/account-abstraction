...
describe('BaasWallet', function () {
  ...
  let BaasWallet1: BaasWallet
  let BaasWallet2: BaasWallet
  let blsSigner1: any
  let blsSigner2: any
  ...
  before(async function () {
    ...
    ({ proxy: BaasWallet1 } = await createBaasWallet(
      ethersSigner,
      blsAgg.address,
      accounts[0],
      accounts[1],
    ...
    ({ proxy: BaasWallet2 } = await createBaasWallet(
      ethersSigner,
      blsAgg.address,
      accounts[1],
      accounts[0],
  ...
  it('owner should be able to call transfer', async () => {
    const { proxy: account } = await createBaasWallet(
      ethersSigner,
      blsAgg.address,
      accounts[0],
      accounts[1],
  ...
  it('other account should not be able to call transfer', async () => {
    ...
    await expect(account.connect(ethers.provider.getSigner(1)).execute(accounts[2], ONE_ETH, '0x'))
      .to.be.revertedWith('BaasWallet: Not Owner or EntryPoint')
  })
  ...
  describe('#validateUserOp', () => {
    ...
    before(async () => {
      ...
      ({ proxy: account } = await createBaasWallet(
    ...
    it('should reject same TX on nonce error', async () => {
      await expect(account.validateUserOp(userOp, userOpHash, AddressZero, 1))
        .to.revertedWith('BaasWallet: Invalid nonce')
    })
  })
  ...
  describe('#EntryPoint.simulateValidation with aggregator', () => {
    ...
    let signer3: any
    let deployer: BaasWalletFactory
    const ownerAddr = createAddress()
    const serverAddr = createAddress()

    before(async () => {
      deployer = await new BaasWalletFactory__factory(ethersSigner).deploy(entrypoint.address, blsAgg.address, AddressZero)
  ...
  context('BaasWalletFactory', () => {
    it('sanity: check deployer', async () => {
      const ownerAddr = createAddress()
      const serverAddr = createAddress()
      const deployer = await new BaasWalletFactory__factory(ethersSigner).deploy(entrypoint.address, blsAgg.address, AddressZero)
